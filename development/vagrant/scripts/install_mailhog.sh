...
cat > /etc/php/8.2/mods-available/mailhog.ini <<'EOF'
; Mailhog replacement for sendmail -t -i
sendmail_path = "/opt/mailhog/mailhog sendmail"
EOF

ln -s /etc/php/8.2/mods-available/mailhog.ini /etc/php/8.2/apache2/conf.d/20-mailhog.ini
ln -s /etc/php/8.2/mods-available/mailhog.ini /etc/php/8.2/cli/conf.d/20-mailhog.ini
