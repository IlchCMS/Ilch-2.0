...
class Thumbnail
{
    ...
    public function __destruct()
    {
        if (is_resource($this->im)) {
            deprecatedImageDestroy($this->im);
        }
        if (is_resource($this->thumb)) {
            deprecatedImageDestroy($this->thumb);
        }
        if (is_resource($this->newimage)) {
            deprecatedImageDestroy($this->newimage);
        }
    }
     ...
     * @param string $output
     */
    public function Createthumb($filename = "unknown", string $output = "screen")
    {
     ...
     */
    private function invalidimage(string $message)
    {
    ...
    private function addpngborder()
    {
        if (file_exists($this->Borderpng)) {
            ...
            deprecatedImageDestroy($borderim);
        }
    }
     ...
     *
     */
    private function rgbpixel($x, $y): array
    {
        if ($x < 0) {
            $x = 0;
        }
    ...
    */
     ...
     *
     */
    private function bilinear($xnew, $ynew): array
    {
        $xf = floor($xnew);
        $xc = $xf + 1;
        $fracx = $xnew - $xf;
     ...
     * @param string $delay
     */
    public function Create_apng(array $frames, string $outputFilename, string $delay)
    {
        $imageData = [];
        $IHDR = [];
        $sequenceNumber = 0;
     ...
     * @return string
     */
    private function create_chunk(string $type, string $data = ''): string
    {
     ...
     * @return string
     */
    private function create_fcTL(int $frameNumber, string $width, string $height, string $delay): string
    {
        $fcTL = [];
     ...
     * @param int $filetype IMAGETYPE_XXX
     * @param string $bgcolor background color
     * @param boolean $transparent transparent
     */
    public function Createcanvas(int $width, int $height, int $filetype, string $bgcolor, bool $transparent)
    {
    ...
    public function Createbase64(string $filename = "unknown"): string
    {
        ...
        ob_end_clean();
        return $encoding . chunk_split(base64_encode($imagecode)) . '"';
    }
     ...
     *
     * @param string $src_image source image
     * @param string $des_image destination image
     */
    public function wipe_exif(string $src_image, string $des_image)
    {
     ...
     * @param string $src_image source image
     */
    public function read_exif(string $src_image): string
    {
        $exif = "";
        if (file_exists($src_image)) {
            $src_file = fopen($src_image, 'rb');
     ...
     *
     * @param string $src_image source image
     * @param string $exifdata exif data in binary format
     */
    public function insert_exif(string $src_image, string $exifdata)
    {
        if (file_exists($src_image)) {
            $src_file = fopen($src_image, 'rb');
            if ($src_file) {
