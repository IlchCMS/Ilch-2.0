...
<table class="table table-hover">
    ...
    <thead>
        <tr>
            ...
            <th></th>
            <th></th>
            <th><?php echo $this->getTrans('articleTitle'); ?></th>
            <?php
                if ($this->get('multilingual')) {
    ...
    <tbody>
        ...
        ?>
            <tr>
                ...
                <td>
                    <a target="_blank" href="<?=$this->getUrl().'/index.php/'.$this->escape($article->getPerma())?>"><?=$article->getTitle()?></a>
                </td>
        <?php
            if ($this->get('multilingual')) {
                echo '<td class="text-right">';
                    foreach ($this->getTranslator()->getLocaleList() as $key => $value) {
                        ...
                        if ($this->get('articleMapper')->getArticleByIdLocale($article->getId(), $key) != null) {
                            echo '<a href="'.$this->getUrl(array('action' => 'treat', 'id' => $article->getId(), 'locale' => $key)).'"><i class="fa fa-edit"></i></a>';
                        } else {
                            echo '<a href="'.$this->getUrl(array('action' => 'treat', 'id' => $article->getId(), 'locale' => $key)).'"><i class="fa fa-plus-circle"></i></a>';
                        }
