...
function rec($id, $uid, $req, $obj)
{
    ...
    foreach ($fk_comments as $fk_comment) {
        ...
        if ($config->get('comment_avatar') == 1) {
            echo '<figure class="thumbnail"><a href="'.$obj->getUrl(array('module' => 'user', 'controller' => 'profil', 'action' => 'index', 'user' => $user->getId())).'"><img class="img-responsive" src="'.$obj->getBaseUrl($user->getAvatar()).'" alt="'.$user->getName().'"></a></figure>';
        }
        echo '</div><div class="col-md-'.$col.' col-sm-'.$col.'"><div class="panel panel-default arrow left"><div class="panel-bodylist"><div class="panel-heading right">'.$user_rep->getName().' <i class="fa fa-reply"></i> '.$translator->trans('replyof').'</div><header class="text-left">';
