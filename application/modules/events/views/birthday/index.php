...
/* 
ToDo
- Zählen wieviele User im Monat Bday haben - x weitere Personen
- Wenn keiner Geburtstag hat in einen Monat dann schreiben das diesen Monat keiner Bday hat.
*/
...
<table class="table table-striped table-responsive">
    <tr>
        <th colspan="2"><div align="left" style="float: left;">Geburtstage von heute</div><div align="right"><?=$this->get('birthdayDateNow') ?></div></th>
    </tr>
        ...
        <tr>
            ...
            <td>
                <div style="margin-top: 3px;">
                    ...
                    <?=$this->getTranslator()->trans('will') ?> <?=floor(($this->get('birthdayDateNowYMD') - str_replace("-", "", $this->escape($birthdaylist->getBirthday()))) / 10000) ?> <?=$this->getTranslator()->trans('yearsOld') ?><br />
                    <?php if($this->getUser() AND $this->getUser()->getId() != $this->escape($birthdaylist->getID())) { ?><a href="<?=$this->getUrl('user/panel/dialognew/id/' . $birthdaylist->getId()) ?>"><?=$this->getTranslator()->trans('writeCongratulations') ?></a><?php } ?>
                </div>
            </td>
        </tr>
    <?php endforeach; ?>
...
<?php foreach ($this->get('birthdayList') as $birthdaylist): ?>
    <?php if ($this->escape(date('n', strtotime("+$x months", strtotime($date->format('Y-m-d'))))) == $this->escape(date('n', strtotime($birthdaylist->getBirthday())))): ?>
        <?php $monthsUserCount++; ?>
    <?php endif; ?>
<?php endforeach; ?>
    <table class="table table-striped table-responsive">
    <tr>
        <th>
            <div align="left" style="float: left;">
                <?php if ($x == 0): ?>
                    <?=$this->getTranslator()->trans('laterIn') ?> 
                <?php endif; ?>
                <?=$months[date('n', strtotime("+$x months", strtotime($date->format('Y-m-d'))))] ?></div>
            <div align="right">
                <?=$monthsUserCount; ?> weitere Personen
                <?php $monthsUserCount = 0; ?>
            </div>
        </th>
    </tr>
