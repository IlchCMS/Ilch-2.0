                ...
                <div class="clearfix">
                    <div class="pull-left">
                        ...
                        if (empty($layout->getIlchCore())) {
                            ...
                            $ilchCoreTooOld = !$layout->hasCoreVersion($coreVersion);
                        }

                        $layoutUpdate = [];

                        $layoutModel = new \Modules\Admin\Models\Layout();
                        foreach ($layoutsOnUpdateServer as $id => $layoutOnUpdateServer) {
                            if ($layoutOnUpdateServer['key'] == $layout->getKey()) {
                                $layoutModel = new \Modules\Admin\Models\Layout();
                                $layoutModel->setByArray($layoutOnUpdateServer);
                                $layoutUpdate['updateserver'] = $layoutModel;

                                unset($layoutsOnUpdateServer[$id]);
                                break;
                            }
                        }

                        if (isset($layoutUpdate['updateserver']) && $layout->isNewVersion($layoutUpdate['updateserver']->getVersion())) : ?>
                            ...
                            <?php else : ?>
                                <form method="POST" action="<?=$this->getUrl(['action' => 'update', 'key' => $layout->getKey(), 'version' => $layout->getVersion(), 'newVersion' => $layoutUpdate['updateserver']->getVersion(), 'from' => 'index']) ?>">
                                    <?=$this->getTokenField() ?>
                                    <button type="submit"
