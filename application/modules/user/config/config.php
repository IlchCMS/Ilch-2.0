    ...
    public function getUpdate(string $installedVersion): string
    {
        switch ($installedVersion) {
            ...
            case "2.2.4":
                ...
                break;
            case "2.2.5":
                // Delete orphaned rows in user_friends and users_dialog as this might have failed with version 2.2.5.
                // Create FKCs if they don't exist.
                $idsUsers = $this->db()->select('id')
                    ->from('users')
                    ->execute()
                    ->fetchList();
                ...
                // Delete orphaned rows in users_dialog.
                $userOneUsersDialog = $this->db()->select('user_one')
                    ->from('users_dialog')
                    ->execute()
                    ->fetchList();
