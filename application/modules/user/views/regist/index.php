<?php if ($this->get('regist_accept') == '1') { ?>
    <form class="form-horizontal" method="POST" action="<?php echo $this->url(array('action' => $this->getRequest()->getActionName())); ?>">
        ...
        <label class="checkbox inline <?php if ($this->get('error') != '') { echo 'text-danger'; } ?>">
            <input type="checkbox" name="acceptRule" value="1"> <?php echo $this->trans('acceptRule'); ?>
        </label>
        <button type="submit" name="save" class="btn pull-right"><?php echo $this->trans('nextButton'); ?></button>
    </form>
<?php }else{
